...
resource "azurerm_app_service_slot" "slots" {
  ...
  key_vault_reference_identity_id = can(each.value.key_vault_reference_identity.key) ? var.combined_objects.managed_identities[try(each.value.identity.lz_key, var.client_config.landingzone_key)][each.value.key_vault_reference_identity.key].id : try(each.value.key_vault_reference_identity.id, null)

  dynamic "site_config" {
    for_each = lookup(each.value, "site_config", {}) != {} ? [1] : []
    ...
    content {
      ...
      java_container            = lookup(each.value.site_config, "java_container", null)
      java_container_version    = lookup(each.value.site_config, "java_container_version", null)
      ...
      min_tls_version           = lookup(each.value.site_config, "min_tls_version", "1.2")
      php_version               = lookup(each.value.site_config, "php_version", null)
      python_version            = lookup(each.value.site_config, "python_version", null)
      ...
      use_32_bit_worker_process = lookup(each.value.site_config, "use_32_bit_worker_process", false)
      ...
      dynamic "ip_restriction" {
        for_each = lookup(each.value.site_config, "ip_restriction", {}) != {} ? [1] : []

        content {
          ip_address                = lookup(each.value.site_config.ip_restriction, "ip_address", null)
          virtual_network_subnet_id = lookup(each.value.site_config.ip_restriction, "virtual_network_subnet_id", null)
        }
      }
  ...
  dynamic "auth_settings" {
    ...
    content {
      ...
      additional_login_params        = lookup(each.value.auth_settings, "additional_login_params", null)
      ...
      dynamic "active_directory" {
        ...
        content {
          client_id         = can(each.value.auth_settings.active_directory.client_id_key) ? var.azuread_applications[try(each.value.auth_settings.active_directory.client_id_lz_key, var.client_config.landingzone_key)][each.value.auth_settings.active_directory.client_id_key].application_id : each.value.auth_settings.active_directory.client_id
          client_secret     = can(each.value.auth_settings.active_directory.client_secret_key) ? var.azuread_service_principal_passwords[try(each.value.auth_settings.active_directory.client_secret_lz_key, var.client_config.landingzone_key)][each.value.auth_settings.active_directory.client_secret_key].service_principal_password : try(each.value.auth_settings.active_directory.client_secret, null)
