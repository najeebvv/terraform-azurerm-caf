...
module "keyvaults" {
  ...
  resource_group      = local.combined_objects_resource_groups[try(try(each.value.resource_group.lz_key, each.value.lz_key), local.client_config.landingzone_key)][try(each.value.resource_group_key, each.value.resource_group.key)]
